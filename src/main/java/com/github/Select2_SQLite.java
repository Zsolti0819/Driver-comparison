...
public class Select2_SQLite {

    public static void main(final String[] args) {
        try (final Connection connection = DriverManager.getConnection(Constants.SQLITE_CONNECTION_STRING);
             final Statement statement = connection.createStatement()) {
            final ResultSet resultSet = statement.executeQuery("SELECT Dealership.dealership_id as d_id1, email, phone_number, address, sale_id, Sales.car_id AS c_id1, owner_id, Sales.dealership_id AS d_id2, sale_date, sale_price, Car.car_id AS c_id2, make, model, year, color, transmission_type, fuel_type, engine_size, number_of_doors FROM Dealership INNER JOIN Sales ON Dealership.dealership_id = Sales.dealership_id INNER JOIN Car ON Sales.car_id = Car.car_id WHERE make = 'Toyota' AND model = 'Supra';\n");
