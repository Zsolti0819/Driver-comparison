...
public class Select1_JFSQL {

    public static void main(final String[] args) {
        try (final Connection connection = DriverManager.getConnection(Constants.JFSQL_CONNECTION_STRING);
             final Statement statement = connection.createStatement()) {
            final ResultSet resultSet = statement.executeQuery("SELECT * FROM Car INNER JOIN Sales ON Car.car_id = Sales.car_id INNER JOIN Owner ON Sales.owner_id = Owner.owner_id WHERE first_name = 'Nick' AND last_name = 'Mertz';");
            ...
            while (resultSet.next()) {
                ...
                ownerIdsFromSales.add(resultSet.getInt("Sales.owner_id"));
                ...
                ownerIdsFromOwner.add(resultSet.getInt("Owner.owner_id"));
