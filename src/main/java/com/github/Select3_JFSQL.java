...
public class Select3_JFSQL {

    public static void main(final String[] args) {
        try (final Connection connection = DriverManager.getConnection(Constants.JFSQL_CONNECTION_STRING);
             final Statement statement = connection.createStatement()) {
            final ResultSet resultSet = statement.executeQuery("SELECT Car.car_id, make, model, year, color, transmission_type, fuel_type, engine_size, number_of_doors, sale_id, Sales.car_id, owner_id, dealership_id, sale_date, sale_price FROM Car LEFT OUTER JOIN Sales ON Car.car_id = Sales.car_id WHERE Sales.car_id = 1;\n");
